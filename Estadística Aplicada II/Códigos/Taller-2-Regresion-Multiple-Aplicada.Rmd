...
La suma de cuadrados de la regresión debida a $\beta_2$ dado que $\beta_1$ ya esta en el modelo es:
$$SSR(\beta_2|\beta_1)=SSR(\beta)-SSR(\beta_1)$$
Conocida como suma extra de cuadrados debido a $\beta_2$, y dado que queremos probar $H_0 : \beta_2 =0$ se construye el siguiente estadistico.

$F_0 = \frac{\frac{SSR(\beta_2|\beta_1)}{r}}{\frac{SE}{n-p}}$

Si $H_0$ es cierta entonces $F_0$ ~ $F_{r,n-p}$

Se realiza la respectiva prueba con la funcion anova, asumiento que el modelo reducido es el $\beta_5$ asociado al NIR29
